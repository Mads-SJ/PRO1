	...
	private void okAction() {
		...
		if (name.length() == 0) {
			...
			if (wage < 0) {
				...
				try {
					employmentYear = Integer.parseInt(txfEmploymentYear.getText());
				} catch (NumberFormatException ex) {
					// do nothing
				}
				...
				if (employee != null) {
					...
					if (companyIsSelected) {
						...
						Controller.setEmploymentYearForEmployee(employee, employmentYear);
					} else {
						Controller.removeEmployeeFromCompany(employee, employee.getCompany());
						Controller.setEmploymentYearForEmployee(employee, 0);
					}
				} else {
					if (companyIsSelected) {
						if (employmentYear == -1) {
							lblError.setText("Please write your employment year.");
						} else {
